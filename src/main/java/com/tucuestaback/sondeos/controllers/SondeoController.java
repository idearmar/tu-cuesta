...
public class SondeoController {
    ...
    @GetMapping("/sondeos")
    public List<SondeoModel> mostrar(){
        return service.traerTodos();
    }

    @PostMapping("/sondeos")
    public ResponseEntity<Map<String, String>> actualizar(@Valid @RequestBody SondeoModel sondeo) {
        this.service.guardarSondeo(sondeo);
        Map<String, String> respuesta = new HashMap<>();
        respuesta.put("mensaje", "Se actulizo el sondeo satisfactoriamente");
        respuesta.put("estado", "true");
        return ResponseEntity.ok(respuesta);
    }
}
